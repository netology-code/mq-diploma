...
В рамках проекта сверстанные макеты должны корректно отображаться минимум на одной из операционных десктопных систем и одной из мобильных:
- компьютер с операционной системой Windows или Mac OS,
- планшеты и смартфоны с операционной системой iOS или
- планшеты и смартфоны с операционной системой Android.

На этих устройствах требуется (в зависимости от наличия), чтобы верстка корректно работала в следующих браузерах:
...
- Последняя версия Mobile Firefox.

В идеале у верстальщика должны быть реальные устройства. Большие компании могут позволить держать парк разных устройств для теста. Остальные могут просто оплатить один из сервисов, которые позволяют тестировать сайты на разных устройствах удаленно или в эмуляторах. Это не идеальный вариант, так как железо тоже влияет на результат, но часто единственный возможный. Для поиска таких сервисов достаточно погуглить "cross browser test online". У некоторых сервисов есть ограниченная бесплатная версия, но как правило они стоят денег.
...
Кроме использования семантических тегов, также нужно правильно вкладывать теги в соответствии с [контентной моделью HTML](https://html.spec.whatwg.org/multipage/dom.html#kinds-of-content). Например, следующий код будет невалидным, то есть ошибочным:
...
```
### Валидная верстка
После полной реализации верстки необходимо ее протестировать с помощью сервиса [W3C Markup Validation Service](https://validator.w3.org). В итоговом отчете не должно быть ошибок или предупреждений. Кроме того существует [новая версия валидатора](https://validator.w3.org/nu/). Она активно разрабатывается и в ней могут быть ошибки или неточности, но многие ее уже используют.
...
Первые три инструмента использовали не по назначению. Теперь на их место пришли гораздо более удобные и современные: флексбокс и гриды. Но гриды вы будете проходить позже.
